...
function queueTime(customers, n) {
  ...
  if (n === 1) {
    return customers.reduce((cur, acc) => cur + acc); // += ?
  }
  if (Math.max(...customers) < n) return Math.max(...customers);

  // Create an Array of 0's with length (n) of tills
  const tills = new Array(n).fill(0);
  for (let customer of customers) {
    //  Add customer to lower line
    tills[tills.indexOf(Math.min(...tills))] += customer;
  }

  //   Return the highest line value
